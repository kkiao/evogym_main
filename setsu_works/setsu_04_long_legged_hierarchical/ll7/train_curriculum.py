...
from ll7.body import BODY_NAME, BODY_WIDTH_VOXELS, make_body
from ll7.curriculum import CURRICULUM_LEVELS, COURSE_VERSION, get_course
from ll7.environment import REWARD_VERSION, SIGHT_DISTANCE
...
def parse_args():
    parser = argparse.ArgumentParser(description="使用PPO训练长腿身体的一个新课程阶段。")
    ...
    parser.add_argument("--network-width", type=int, choices=(64, 128), default=128)
...
class EvaluationCallback(BaseCallback):
    ...
    def _save_and_evaluate(self):
        ...
        print(
            f"[long_legged L{self.args.level}] step={self.num_timesteps} "
...
def transfer_policy(source_path: Path, target_model):
    ...
    if source.observation_space.shape == target_model.observation_space.shape:
        target_model.policy.load_state_dict(source_state)
        return

    source_dim = int(source.observation_space.shape[0])
    target_dim = int(target_model.observation_space.shape[0])
    if (source_dim, target_dim) != (72, 97):
        raise ValueError(
            f"不支持从观测维度 {source_dim} 迁移到 {target_dim}。"
        )
    ...
    for key, source_value in source_state.items():
        if key in first_layers:
            ...
            if source_value.shape[0] != target_value.shape[0]:
                raise ValueError("旧模型与新模型的隐藏层宽度不一致。")
            target_value.zero_()
            target_value[:, :51] = source_value[:, :51]
            target_value[:, 61:82] = source_value[:, 51:72]
        elif source_value.shape == target_state[key].shape:
            target_state[key] = source_value
        else:
            raise ValueError(f"无法迁移参数 {key}：网络结构不一致。")
    ...
    if args.resume:
        ...
        write_json(
            run_dir / "config.json",
            {
                ...
                "sight_distance_voxels": SIGHT_DISTANCE,
